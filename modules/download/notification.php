...
if ($data['type'] == 'report_new') {
    $data['title'] = sprintf($nv_Lang->getModule('notification_report'), $data['send_from'], $data['content']['title']);
    ...
    $data['title'] = sprintf($nv_Lang->getModule('notification_upload'), $data['send_from'], $data['content']['title']);
